...
export function validates(validators: ValidationChain[], allowExtraFields: Boolean = false) {
    return async (req: Request, res: Response, next: NextFunction) => {
        if (!allowExtraFields) {
            ...
            if (isExtraFieldExists) 
                next(new BadRequestError("Only whitelisted keys are allowed", allowedFields));
...
function extraFieldCheck(validators: ValidationChain[], req: Request): {
    isExtraFieldExists: boolean,
    allowedFields: string[]
} {
    // allowrd fields extractions from validators
    const allowedFields: string[] = validators.reduce((fields: any, rule: any) => {
        return [...fields, ...rule.builder.fields]
    }, []).sort().map((f: string) => f.split(".")[0]);
